...
await test('D17','15-minute mode preserved; arming does not touch football fields',()=>{const g=armed();g.quarterLengthSeconds=900;g.scoreboardSeconds=900;const old=JSON.stringify([g.rngState,g.possession,g.awayPower,g.homePower]);dm.tick(g,due);assert.equal(g.scoreboardSeconds,900);assert.equal(old,JSON.stringify([g.rnggState,g.possession,g.awayPower,g.homePower]).replace('null','null'));});
