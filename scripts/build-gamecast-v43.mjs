...
for(const p of [path.join(srcFn,'index.ts'),path.join(srcFn,'power.ts'),path.join(srcFn,'week3.ts'),path.join(srcUi,'app.js'),path.join(srcUi,'index.html'),week4Source]){
  if(!fs.existsSync(p))throw new Error(`Required 4.3 build input missing: ${path.relative(root,p)}`);
}

// Backend: copy the accepted 4.2.2 football engine used by the 4.2.3 UI, then change only weekly identity/binding.
...
const priorPower=read(path.join(srcFn,'power.ts'));
...
index=once(index,'W3-51+W2-G0021+POWER_60_99_V422+PERIOD_STATE_FIX+PLAYVOL_CAL_2500','W4-55+SCHEDULE_V5+TV54_EXACT_1_BLOCKED+POWER_V422_CARRYFWD+V423_UI','data version');
...
if(read(path.join(dstFn,'power.ts'))!==priorPower)throw new Error('4.3 build unexpectedly changed carried-forward power.ts');

// UI: exact 4.2.3 UI behavior, rebound to the isolated Week 4 / 4.3 backend and storage namespace.
...
app=once(app,'// UI 4.2.3 retains the 4.2.2 backend, engine identity and session storage.','// UI 4.3 retains the 4.2.3 interaction hardening and the 4.2.2 football mechanics; only Week 4 binding/identity changes.','UI provenance comment');
...
page=once(page,'WEEK 3 + W2 CARRYOVER · 60–99 POWER RELEASE · 4.2 RC3 MECHANICS FROZEN','WEEK 4 · 55-GAME OPERATING SLATE · 4.2.3 UI + 4.2.2 FOOTBALL MECHANICS','page subtitle');
...
const newNotice='<div class="notice operator-only"><b>4.3 BUILD GOVERNANCE</b> · 55 canonical Week 4 games from the locked schedule. 54 carriage rows are exact immutable-master joins. G0142 Colorado at Northwestern remains TV-carriage BLOCKED because the quarantined source row says Colorado at Nebraska; its kickoff/network are intentionally shown as TBD pending upstream authority. Football/play/timing logic is unchanged from the 4.2.2 cloud engine, and Chairman interaction hardening is unchanged from the 4.2.3 UI. The prior 4.2.2 power sidecar is carried forward without edits; AUTO remains blocked for games missing current OFF/DEF/OVERALL ratings. <a href="../v4.2.1/review.html">OPEN POWER REVIEW</a>.</div>';
page=once(page,oldNotice,newNotice,'governance notice');
page=once(page,'<summary>WEEK 3 POWER INTAKE</summary><p class="sub">Cloud session is preloaded with the 4.2.2 Week 3 power package. Manual updates remain an operator control and should only be used when explicitly authorized.</p>','<summary>WEEK 4 POWER INTAKE</summary><p class="sub">The prior 4.2.2 power sidecar is carried forward unchanged as a safe default. Week 4 rating updates remain an explicit Chairman-controlled load and AUTO stays blocked where either team is power-pending.</p>','power intake copy');
...
console.log(JSON.stringify({backend:path.relative(root,dstFn),ui:path.relative(root,dstUi),engine:'GC-W4-V4.3-RC1',week:'2026-W04',games:55},null,2));
