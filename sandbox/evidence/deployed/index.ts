...
import { OPERATING_SLATE } from "./week4.ts";
import { TEAM_POWER, type TeamPowerInput } from "./power.ts";

const ENGINE_VERSION = "GC-W4-V4.3.1-RC1";
const DATA_VERSION = "W4-55+SCHEDULE_V5+TV54_EXACT_1_BLOCKED+POWER_W4_POST_W3_121_60_99+GAMECAST_V1_1_FCS60+V431_UI_RESET";
const WEEK_KEY = "2026-W04";
const CONSOLE_URL = "https://rgbslb11.github.io/Synthcast-Console/v4.3.1/";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (action === "create" && req.method === "POST") {
      if (OPERATING_SLATE.length !== 55) throw new Error("4.3 operating slate count mismatch");
      const operator = randHex(32), publicSlug = `w4v431-${randHex(8)}`, state = OPERATING_SLATE.map(initialGame);
      ...
      return json({ ...row, engine_version: ENGINE_VERSION, operator_token: operator, master_zulu: now(), governance: { games: state.length, week4_games: state.filter(g => g.canonicalWeek === "W4").length, power_ready: state.filter(g => g.powerReady).length } }, 201);
    }
    ...
    if (!cur || cur.week_key !== WEEK_KEY || !slug.startsWith("w4v431-")) return json({ error: "GameCast 4.3.1 Week 4 session not found" }, 404);
    ...
    if (action === "power") {
      ...
      await client.rpc("gamecast_v12_insert_event", { p_session_id: cur.session_id, p_state_version: up.state_version, p_event_type: "V431_POWER_LOAD", p_game_id: null, p_payload: { ...summary, sourceVersion: body.sourceVersion ?? null, masterZulu: stamp } });
    ...
    await client.rpc("gamecast_v12_insert_event", { p_session_id: cur.session_id, p_state_version: up.state_version, p_event_type: `V431_${c.toUpperCase()}`, p_game_id: g.id, p_payload: event });
    return json({ state: up.state, state_version: up.state_version, master_zulu: stamp, engine_version: ENGINE_VERSION });
  } catch (e: any) {
    console.error("gamecast-week4-v4-3-1", e?.message ?? e);
