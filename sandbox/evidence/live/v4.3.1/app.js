'use strict';
const API='https://percrnamjzetzjjuxuuw.supabase.co/functions/v1/gamecast-week4-v4-3-1',STORE='synthcastGameCast431Operator',qs=new URLSearchParams(location.search);let slug=qs.get('session')||'',game=qs.get('game')||'',token=location.hash.startsWith('#operator=')?location.hash.slice(10):'',publicView=qs.get('view')==='public',state=[],version=0,filter='ALL',loading=false,engine='GC-W4-V4.3.1-RC1',masterEpoch=Date.now(),masterAnchor=Date.now();const root=document.getElementById('root');if(publicView){root.classList.add('public');document.body.classList.add('public');document.getElementById('modeLabel').textContent='PUBLIC LIVE SCOREBOARD';document.querySelector('.public-only').hidden=false}
...
// UI 4.3 retains the 4.2.3 interaction hardening and the 4.2.2 football mechanics; only Week 4 binding/identity and the authorized Week 4 power package change.
