...
function adopt(b){
  state=b.state||[];version=b.state_version??version;engine=b.engine_version||engine;
...
async function load({background=false,force=false}={}){
  ...
  readPromise=(async()=>{
    try{
      ...
      updateReadClocks(b);
      if(interactionLocked()||writeConflict){
      ...
      adopt(b);render();
      if(!background||se().classList.contains('error'))status('CLOUD V'+version+' · POWER '+(b.governance?.power_ready??0)+'/'+(b.governance?.games??state.length));
...
async function cmd(id,command,payload={}){
  ...
  try{
    ...
    if(e.status===409&&e.message==='version conflict'){writeConflict=true;status('VERSION CONFLICT · Draft preserved · REFRESH to discard drafts and reload',true)}
    else status('WRITE ERROR · '+e.message,true);
  }finally{writing=false}
  if(readback)await load({force:true});
}
function confirmCmd(id,c,m,p={}){if(confirm(m))cmd(id,c,{...p,confirmed:true})}function beginEdit(id,strong=false){if(strong&&!confirm('Supersede this locked/accepted result for correction?'))return;cmd(id,'edit_begin',strong?{confirmed:true}:{})}function reopen(id){confirmCmd(id,'reopen_live','Reopen this Final to live simulation? A continuation seed will be created.',{resumeAuto:false})}function parseClock(v){const m=String(v).match(/^(\d{1,2}):(\d{2})$/);return m?+m[1]*60+ +m[2]:NaN}function setClock(id){const g=state.find(x=>x.id===id),v=prompt('Set clock M:SS',clk(g.scoreboardSeconds));if(v==null)return;const s=parseClock(v);if(!Number.isFinite(s)||s<0||s>g.quarterLengthSeconds)return alert('Invalid clock');cmd(id,'clock_set',{seconds:s})}function saveEdit(id,resumeAuto){const scores={};for(const p of ['1','2','3','4'])scores[p]=[+document.getElementById('e-'+id+'-a'+p).value,+document.getElementById('e-'+id+'-h'+p).value];const c=parseClock(document.getElementById('e-'+id+'-c').value);if(!Number.isFinite(c))return alert('Clock must be M:SS');cmd(id,'edit_commit',{scores,quarter:+document.getElementById('e-'+id+'-q').value,clockSeconds:c,possession:+document.getElementById('e-'+id+'-p').value,ballSide:document.getElementById('e-'+id+'-s').value,yardline:+document.getElementById('e-'+id+'-y').value,down:+document.getElementById('e-'+id+'-d').value,distance:+document.getElementById('e-'+id+'-x').value,awayTimeouts:+document.getElementById('e-'+id+'-at').value,homeTimeouts:+document.getElementById('e-'+id+'-ht').value,resumeAuto,hold:!resumeAuto})}function openGame(id){location.href=location.origin+location.pathname+'?session='+encodeURIComponent(slug)+'&game='+id+(publicView?'&view=public':'')+(publicView?'':'#operator='+token)}function copy(u){navigator.clipboard?.writeText(u).catch(()=>prompt('Copy link',u))}async function loadPower(){
  if(writing)return;
  if(writeConflict){status('VERSION CONFLICT · Draft preserved · REFRESH to reload',true);return}
  writing=true;readEpoch++;
  try{
    ...
    if(e.status===409&&e.message==='version conflict'){writeConflict=true;status('VERSION CONFLICT · Draft preserved · REFRESH to reload',true)}
    else status('POWER ERROR · '+e.message,true);
  }finally{writing=false}
}

if(!publicView){document.getElementById('create').onclick=async()=>{try{const b=await api('create',{});slug=b.public_slug;token=b.operator_token;version=b.state_version||0;anchorMaster(b.master_zulu);history.replaceState(null,'',location.pathname+'?session='+slug+'#operator='+token);saveSession();await load()}catch(e){status('CREATE ERROR · '+e.message,true)}};document.getElementById('resume').onclick=()=>{const s=JSON.parse(localStorage.getItem(STORE)||'null');if(s?.slug&&s?.token){slug=s.slug;token=s.token;history.replaceState(null,'',location.pathname+'?session='+slug+'#operator='+token);load()}};document.getElementById('refresh').onclick=refresh;document.getElementById('copyPublic').onclick=()=>copy(location.origin+location.pathname+'?session='+slug+'&view=public');document.getElementById('copyOperator').onclick=()=>copy(location.origin+location.pathname+'?session='+slug+'#operator='+token);document.getElementById('loadPower').onclick=loadPower}else document.getElementById('refreshPublic').onclick=refresh;document.querySelectorAll('[data-f]').forEach(b=>b.onclick=()=>{filter=b.dataset.f;game='';document.querySelectorAll('[data-f]').forEach(x=>x.classList.toggle('active',x===b));render()});if(!publicView&&!slug){const s=JSON.parse(localStorage.getItem(STORE)||'null');if(s?.slug&&s?.token){slug=s.slug;token=s.token;history.replaceState(null,'',location.pathname+'?session='+slug+'#operator='+token)}}tick();setInterval(tick,1000);load();setInterval(()=>{if(slug&&!loading&&!writing)load({background:true})},publicView?4000:7000);
