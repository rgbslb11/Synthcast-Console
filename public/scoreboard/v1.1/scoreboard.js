...
(() => {
  ...
  const ENGINE='GC-W5-SAT-V4.3.2.2-RC1';
  ...
  function updateSummary(){
    ...
    document.getElementById('weekLabel').textContent='WEEK 5 · SATURDAY';
  }
  ...
  async function read(){
    ...
    try{
      ...
      if(data.public_slug!==slug||data.week_key!=='2026-W05'||data.engine_version!==ENGINE||!Array.isArray(data.state)||data.state.length!==49||data.state.some(g=>g.date!=='2026-09-26'))throw new Error('Week 5 Saturday session mismatch');
      state=data.state;
