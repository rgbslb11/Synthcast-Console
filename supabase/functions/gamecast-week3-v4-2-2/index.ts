...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!cur || cur.week_key !== WEEK_KEY || !slug.startsWith("w3v422-")) return json({ error: "GameCast 4.2.2 Week 3 session not found" }, 404);
    ...
    if (action === "power") {
      ...
      await client.rpc("gamecast_v12_insert_event", { p_session_id: cur.session_id, p_state_version: up.state_version, p_event_type: "V422_POWER_LOAD", p_game_id: null, p_payload: { ...summary, sourceVersion: body.sourceVersion ?? null, masterZulu: stamp } });
    ...
    await client.rpc("gamecast_v12_insert_event", { p_session_id: cur.session_id, p_state_version: up.state_version, p_event_type: `V422_${c.toUpperCase()}`, p_game_id: g.id, p_payload: event });
    return json({ state: up.state, state_version: up.state_version, master_zulu: stamp, engine_version: ENGINE_VERSION });
  } catch (e: any) {
    console.error("gamecast-week3-v4-2-2", e?.message ?? e);
